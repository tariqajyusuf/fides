...
def aircall_identity_email(saas_config):
    return (
        pydash.get(
            saas_config, "aircall.identity_email") or secrets["identity_email"]
    )
...
def aircall_create_erasure_data(
    ...
    api_key = aircall_secrets["api_key"]
    ...
    headers = {"Authorization": f"basic {aircall_secrets['api_key']}"}

    users_response = requests.post(
        url=f"{base_url}/v1/contacts", headers=headers, json=body)
